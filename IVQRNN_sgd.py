#Code used to run simulations for the IVQRNN model using the SGD with momentum estimation approach
import os
import uuid
import numpy as np
import pandas as pd
...
class QuantileSemiparametricNN(nn.Module):
    def __init__(self, input_dim: int, sample_size: int, dhat_index: int, q: float = 0.5, m: int = 1):
        ...
        self.activation = HybridActivation(m=1)
...
def gen_ivqr_eg_mixed3(n=10**4, k=10, m=0, s=10, nonlinear=True, seed=None):
    ...
    d = z * (u > 0.5*v)  # Threshold-based treatment    
    alpha_true = 1.0 * (u - 0.5)  # Varies with quantile    
    beta_lin = np.linspace(0.2, 0.8, num_normal)
    beta_bin = np.random.uniform(-0.5, 0.5, num_binomial)    
    y =  alpha_true*d #    
    x_normal = StandardScaler().fit_transform(x[:, :num_normal])
    if nonlinear:
    ...
    y += norm.ppf(u, 0, 1)  # Smaller noise variance    
...
def vciqrNN(alpha, nonpar_output, y, d, x, z, tau):
    ...
    J = (dx.T @ (zx * kernel_vals)) / (n * h)  # Shape (k+1, m+k)
    try:
        J_inv = np.linalg.pinv(J) 
    except np.linalg.LinAlgError:
        raise RuntimeError("Matrix inversion failed")
    S = (zx.T @ zx) / n  # Shape (m+k, m+k)
