...
def gen_ivqr_eg_mixed3(n=10**4, k=10, m=0, s=10, nonlinear=True, seed=None):
    if seed is not None:
        np.random.seed(seed)
    ...
    for _ in range(num_binomial):
        x_columns.append(np.random.binomial(n=1, p=0.4, size=n))
    x = np.column_stack(x_columns)
    z = np.random.binomial(n=1, p=0.7, size=n)
    v = np.random.normal(size=n)
    d = z * (u > 0.5*v)  
    alpha_true = 1.0 * (u - 0.5)  
    beta_lin = np.linspace(0.1, 0.1, num_normal)
    beta_quad = np.linspace(0.05, 0.2, num_normal)
    beta_bin = np.random.uniform(0.1, 0.1, num_binomial)
...
def fit_for_tau_nn(tau, y_train, D_train, X_train, Z_train,
    ...
    alpha_range = np.linspace(two_sqr[0] - 2*std_2sqr[0], two_sqr[0] + 2*std_2sqr[0], 20)
    
    learning_rate = 0.1
    momentum_param = 0.9
    n_steps = 10
    current_alpha = alpha_range[len(alpha_range) // 2]
    velocity = 0
    best_g = float('inf')
    best_alpha = None
    best_model = None
    delta = alpha_range[1] - alpha_range[0] if len(alpha_range) > 1 else 0
    def train_model_for_alpha(alpha, epochs=100, patience=10):
    ...
    for step in range(n_steps):
        model_current = train_model_for_alpha(current_alpha, epochs=50)
        current_g = abs(model_current.dhat_coefficient.item())
        current_L = current_g ** 2
        if current_g < best_g:
            best_g = current_g
            best_alpha = current_alpha
            best_model = model_current
        alpha_plus = current_alpha + delta
        if alpha_plus > alpha_range[-1] or alpha_plus < alpha_range[0]:
            alpha_plus = current_alpha - delta 
            if alpha_plus < alpha_range[0] or alpha_plus > alpha_range[-1]: 
                break
        model_plus = train_model_for_alpha(alpha_plus, epochs=50)
        g_plus = abs(model_plus.dhat_coefficient.item())
        L_plus = g_plus ** 2
        gradient = (L_plus - current_L) / delta
        velocity = momentum_param * velocity - learning_rate * gradient
        current_alpha += velocity
        current_alpha = np.clip(current_alpha, alpha_range[0], alpha_range[-1])
        if g_plus < best_g:
            best_g = g_plus
            best_alpha = alpha_plus
            best_model = model_plus
    if best_model is None:
        best_alpha = alpha_range[len(alpha_range) // 2] # Fallback to initial if no improvement
        best_model = train_model_for_alpha(best_alpha, epochs=200)
    else:
        best_model = train_model_for_alpha(best_alpha, epochs=200) # Retrain with more epochs
    best_g = abs(best_model.dhat_coefficient.item())
    best_model.eval()
    with torch.no_grad():
        z_train_tensor = torch.tensor(z_train_combined, dtype=torch.float32).to(device)
    ...
    criterion = QuantileLoss(tau)
    y_train_adj = y_train - best_alpha * D_train
    z_train_tensor = torch.tensor(z_train_combined, dtype=torch.float32).to(device)
    y_train_adj_tensor = torch.tensor(y_train_adj, dtype=torch.float32).to(device)
    with torch.no_grad():
        train_outputs = best_model(z_train_tensor).squeeze()
        train_loss = criterion(train_outputs, y_train_adj_tensor).item()
    y_val_adj = y_val - best_alpha * D_val
    z_val_tensor = torch.tensor(z_val_combined, dtype=torch.float32).to(device)
    y_val_adj_tensor = torch.tensor(y_val_adj, dtype=torch.float32).to(device)
    with torch.no_grad():
        val_outputs = best_model(z_val_tensor).squeeze()
        val_loss = criterion(val_outputs, y_val_adj_tensor).item()
    y_test_adj = y_test - best_alpha * D_test
    z_test_tensor = torch.tensor(z_test_combined, dtype=torch.float32).to(device)
    y_test_adj_tensor = torch.tensor(y_test_adj, dtype=torch.float32).to(device)
    with torch.no_grad():
...
def run_simulation():
    data = gen_ivqr_eg_mixed3(n=N_OBS, k=75, m=0.6, s=75, nonlinear=True)
